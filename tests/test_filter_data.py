...
from bank_customer.transform.transform import (
    filter_by_column_equal,
    filter_by_column_over_under,
    get_keep_columns,
)
...
def test_keep_columns_raises_error_for_missing_columns():
    df = clean_heads(extract(get_path_raw("dataset.csv")))

    with pytest.raises(KeyError, match="Missing required columns: missing_column"):
        get_keep_columns(df, ["age", "missing_column"])
